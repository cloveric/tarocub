...
- **v0.1.74** (current) — Cancelling a queued task from its card no longer flickers back to "queued": an in-flight queue-wait notification could overwrite the "cancelled" card right after the tap, so the wait render is now skipped for tasks already cancelled.
