...
- **v0.1.76** (current) — Setting reasoning effort to `max` no longer makes the engine appear to fail: the run card hit Feishu's per-element size limit (ErrCode 11310, measured in bytes — CJK ~3/char) because the collapsed tool-summary and todo-plan panels aggregated unboundedly. Every card element is now byte-capped (~9000 bytes) regardless of effort, tool count, or CJK content.
