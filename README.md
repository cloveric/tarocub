...
- **v0.1.75** (current) — Cancelling a queued task now posts a fresh "已取消" notice and recalls the queue card, instead of patching it in place — Feishu reverts a just-clicked card's async patch back to its pre-click state, which caused the cancel to flicker to "cancelled" then snap back to "queued".
