...
- **v0.1.73** (current) — AskUserQuestion cards gain a free-text "Other" input per question; after submit the form is cleaned up (a compact read-only summary is posted and the now-useless form is recalled, since Feishu won't let a submitted form be patched read-only in place). Cancelling a queued task from its card now always gives immediate feedback instead of silently doing nothing when the card can't be updated in place.
