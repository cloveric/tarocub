...
- **v0.1.78** (current) — `/compact` no longer **resets (destroys) your conversation** when compaction can't run. The handler forwards `/compact` to the engine and, on any non-auth error, used to reset the session and call it "the same effect" — but a reset throws the conversation away, while compaction keeps it (summarized). A failed `/compact` now leaves the session intact and says so ("Couldn't compact … your conversation is unchanged. Send /reset if you want to start fresh."). Verified that Claude's headless stream-json mode *does* support `/compact` (it compacts and returns successfully), so this only changes the failure path; same fix on Telegram and Lark.
- **v0.1.77** — Interactive Lark cards now update **in place after you interact with them**, instead of recalling and reposting. On submit, an AskUserQuestion form flips to a read-only "已提交" card; the queue-cancel card flips to "已取消"; the run card streams and finalizes — all via CardKit managed cards (`cardkit.v1.card.update` by `card_id`) rather than `im.message.patch`. The post-interaction update is also detached from the card-action handler and delayed ~1s (`settleThenUpdateManagedCard`): Feishu's client locks a just-clicked/submitted card for a brief window and reverts any update issued during it — CardKit included — so an immediate update silently snaps back. Every path falls back to the previous recall/notice/patch behavior when CardKit is unavailable. Real-Feishu verified end to end.
- **v0.1.76** — Setting reasoning effort to `max` no longer makes the engine appear to fail: the run card hit Feishu's per-element size limit (ErrCode 11310, measured in bytes — CJK ~3/char) because the collapsed tool-summary and todo-plan panels aggregated unboundedly. Every card element is now byte-capped (~9000 bytes) regardless of effort, tool count, or CJK content.
- **v0.1.75** — Cancelling a queued task now posts a fresh "已取消" notice and recalls the queue card, instead of patching it in place — Feishu reverts a just-clicked card's async patch back to its pre-click state, which caused the cancel to flicker to "cancelled" then snap back to "queued".
- **v0.1.74** — Cancelling a queued task from its card no longer flickers back to "queued": an in-flight queue-wait notification could overwrite the "cancelled" card right after the tap, so the wait render is now skipped for tasks already cancelled.
- **v0.1.73** — AskUserQuestion cards gain a free-text "Other" input per question; after submit the form is cleaned up (a compact read-only summary is posted and the now-useless form is recalled, since Feishu won't let a submitted form be patched read-only in place). Cancelling a queued task from its card now always gives immediate feedback instead of silently doing nothing when the card can't be updated in place.
- **v0.1.72** — Fix AskUserQuestion forms freezing on Lark: the SDK's card-action normalizer drops `form_value`, so dropdown/select picks were read as empty and the bot re-prompted "请先选择" forever while the run card stayed "running". The channel now keeps the raw Feishu event (`includeRawEvent`) and recovers the form values from it.
- **v0.1.71** — The Lark `/help` is rewritten from a 28-bullet wall of text into a quick-start intro plus four compact grouped sections (Session, Settings, Workspace & groups, Advanced); `/newtopic` is listed and the deprecated `/ultrareview` is replaced by `/code-review`, which now also routes through the Lark command path.
- **v0.1.70** — `/newgroup` and `/newtopic` now hand group **ownership to the human operator** (the bot stays a manager), so you can change the group's message form (topic/conversation) and other settings instead of being a plain member.
- **v0.1.69** — Optional Lark scopes are now organized into 12 opt-in feature groups (ordinary group messages, chat admin, doc auto-grant, Sheets, Calendar, Tasks, Base/Bitable, VC/Minutes, Contact, Whiteboards, Slides, Wiki); `lark permissions` and provisioning surface each family with its own bulk-import JSON. The QR registration auto-grants none of them — only messaging, cards, and Docs/Drive are zero-console.
