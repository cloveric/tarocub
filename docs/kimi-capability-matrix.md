...
compatibility baseline is 0.43.0 with ACP TypeScript SDK 1.4.0, where new/load,
Search MCP, full form elicitation, cancellation and reuse, and background Agent
completion were live-verified. The 0.41 permission and detached-question
...
|---|---|---|---|
| Runtime transport | Persistent app-server by default; process fallback | Persistent stream-json worker | **Aligned:** persistent `kimi acp` worker with JSON-RPC framing; Kimi 0.43.0's default `agent-core-v2` path is live-verified with ACP SDK 1.4.0 |
