...
compatibility baseline is 0.40.1, where `session/new` and `session/load` both
...
|---|---|---|---|
| Runtime transport | Persistent app-server by default; process fallback | Persistent stream-json worker | **Aligned:** persistent `kimi acp` worker with JSON-RPC framing; Kimi 0.40.1's default `agent-core-v2` path is live-verified |
...
| Full-auto/bypass | Native sandbox/full-auto and bypass modes | Native permission modes | **Aligned with explicit boundaries:** full-auto -> ACP `yolo` (ordinary tools auto-approved; sensitive commands may still ask); bypass -> ACP `auto` (unattended, with Kimi's dangerous-command guard enabled by default). Neither is an OS sandbox |
