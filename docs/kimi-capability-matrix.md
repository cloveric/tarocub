...
compatibility baseline is 0.41.0, where `session/new` and `session/load` both
live-verified native stdio MCP startup, ACP `auto` became true Never Ask mode,
and background questions gained a detached lifecycle. Remaining gaps must still
be re-probed before removal.

| Capability | Codex | Claude Code | Kimi Code alignment |
|---|---|---|---|
| Runtime transport | Persistent app-server by default; process fallback | Persistent stream-json worker | **Aligned:** persistent `kimi acp` worker with JSON-RPC framing; Kimi 0.41.0's default `agent-core-v2` path is live-verified |
