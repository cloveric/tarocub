...
compatibility baseline is 2.0.1 with ACP TypeScript SDK 1.4.0. New/list/load, a
resumed turn, and a harmless Bash invocation under ACP `yolo` with zero
approval requests were re-verified on 2.0.1. The 2.0.0 probe covered streamed
thought/text, cancellation followed by worker reuse, and local image attachment
reading; the 0.43.0 probe additionally covered Search MCP, full form
elicitation, and background Agent completion. The 0.41 permission and
detached-question semantics remain in force. Remaining gaps must still be
re-probed before removal.

| Capability | Codex | Claude Code | Kimi Code alignment |
|---|---|---|---|
| Runtime transport | Persistent app-server by default; process fallback | Persistent stream-json worker | **Aligned:** persistent `kimi acp` worker with JSON-RPC framing; Kimi 2.0.1's default `agent-core-v2` path is live-verified with ACP SDK 1.4.0 |
...
| Structured questions | Native request-user-input path | Native `AskUserQuestion` | **Aligned for choice forms on Kimi 0.43+:** ACP form elicitation renders complete multi-question and multi-select flows in Lark and Telegram while mapping display labels back to schema values. Kimi 0.41.0 background questions may outlive the foreground turn; TaroCub retains their approval lifecycle, routes late requests through Hook-recorded question tasks, and cancels them on task stop, worker teardown, or expiry. Older permission callbacks retain the single-choice fallback; arbitrary free-text ACP forms are rejected rather than misrendered |
...
| Mid-turn steering | App-server `turn/steer` | No protocol support; later message queues | **Gap:** Kimi 0.43 can interrupt its own background `WaitFor`, but ACP exposes no client steering method and rejects a concurrent prompt; later TaroCub messages therefore queue as separate turns |
...
| MCP servers | Native Codex MCP configuration | Native Claude MCP/plugins | **Aligned on Kimi 0.43+:** native Kimi user/project MCP/plugins remain active, and TaroCub injects its complete configured MCP list, including the Brave/Tavily stdio Search MCP, through ACP for every new or loaded session. Structured MCP results and attachments remain intact, and session initialization fails closed instead of silently omitting search capability. Kimi's new deferred tool loading is not enabled for `cctb_search`, preserving immediate search discovery |
