...
export function larkAgentInstructions(): string {
  const lines = [
    ...
    "Bridge tags: [send-file:/absolute/path], [send-image:/absolute/path], send.file/send.image/send.audio/send.video/send.batch; lark.choice or `request_user_input`; Claude/Kimi `AskUserQuestion` => Feishu card. Do not call `lark-cli` just to send choice cards. Small text: fenced `file:name.ext`. A background job that creates artifacts must print the exact delivery tags in its final stdout; saying only `saved PATH` is not delivery.",
