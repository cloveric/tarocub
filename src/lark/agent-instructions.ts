...
export function larkAgentInstructions(requestText = ""): string {
  const lines = [
    ...
    "Images: title each via send.batch {path,caption} or title line directly above [send-image:]. ONE titled batch -> ONE card. send.batch: max 20 artifacts/120 MiB per call; split larger sets into multiple calls; each path once. Use exactly one syntax per artifact; never repeat a path unless explicitly asked to resend.",
