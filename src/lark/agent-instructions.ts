...
export function larkAgentInstructions(requestText = ""): string {
  const lines = [
    ...
    "Bridge tags: [send-file:/absolute/path], [send-image:/absolute/path], send.file/send.image/send.audio/send.video/send.batch; lark.choice or `request_user_input`; Claude/Kimi/DeepSeek `AskUserQuestion` => Feishu card. Do not call `lark-cli` just to send choice cards. Small text: fenced `file:name.ext`. Background jobs: verify output, not exit status; repair empty/all-zero/corrupt results; final stdout must include exact delivery tags and one user-facing conclusion. `saved PATH` alone is not delivery.",
