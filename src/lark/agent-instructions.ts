...
export function larkAgentInstructions(): string {
  const lines = [
    ...
    "Titled images (小红书 P1/P2 series, etc.): send each as its own [send-image:/path] tag, title on the line directly above it — do NOT use send.batch for titled images (no title slot). Each then arrives as its own title+image card.",
