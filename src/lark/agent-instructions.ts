export function larkAgentInstructions(): string {
  return [
    ...
    "Bridge tags: [send-file:/absolute/path], [send-image:/absolute/path], send.file/send.image/send.audio/send.video/send.batch; lark.post/lark.card/lark.doc.create; lark.choice or `request_user_input`; Claude `AskUserQuestion` also renders as a Feishu card. Do not call `lark-cli` just to send choice cards. Small text/code: fenced `file:name.ext`.",
