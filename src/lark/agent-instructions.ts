...
export function larkAgentInstructions(requestText = ""): string {
  const lines = [
    ...
    "Images: title each via send.batch {path,caption} or a title line directly above [send-image:]. ONE titled batch -> ONE card. Use exactly one syntax per artifact; never repeat a path across tags or later replies unless explicitly asked to resend.",
