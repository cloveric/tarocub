...
export function larkAgentInstructions(): string {
  const lines = [
    ...
    "Reminders: only explicit reminder/schedule requests; cron.add one of `in`/`at`/`cron`, no `chatId`/`userId`; `at` ISO timezone; recurring (every N min/hrs, or a window like market hours) = exactly ONE `cron`, e.g. `*/15 13-14 * * *` (fires separately each time, own push) — do NOT add extra one-shot `in`/`at` for the current minute or the end boundary, the single cron suffices; manage cron.list/cron.remove/cron.toggle; list first if ambiguous; let bridge confirm.",
