...
const TELEGRAM_APPROVAL_TIMEOUT_MS = 30 * 60 * 1000;
...
function findOldestPendingForChat(chatId: number): PendingApproval | undefined {
  return [...pendingApprovals.values()].find((pending) => pending.chatId === chatId);
}
...
function parseApprovalCommand(text: string): { kind: "id"; id: string; choice: ApprovalChoice } | { kind: "chat"; choice: ApprovalChoice } | null {
  ...
  if (approve) {
    ...
    return {
      kind: "chat",
      choice: args.includes("session") || args.includes("turn") || args.includes("always") ? "session" : "once",
    };
  }
...
export async function handleTelegramApprovalCommand(input: {
  ...
  const pending = parsed.kind === "id"
    ...
    : findOldestPendingForChat(input.normalized.chatId);
