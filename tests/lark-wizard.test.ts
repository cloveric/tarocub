  ...
  it("registers a PersonalAgent app and stores credentials in lark.env", async () => {
    ...
    try {
      const envPath = await runLarkWizard(
        { USERPROFILE: tempDir, CCTB_LARK_STATE_DIR: stateDir, LARK_DOMAIN: "feishu", CODEX_TELEGRAM_INSTANCE: "ccfcc1" },
      ...
      expect(saved).toContain('CODEX_TELEGRAM_INSTANCE="ccfcc1"');
